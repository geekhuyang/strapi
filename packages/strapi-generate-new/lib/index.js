...
const { defaults, merge, pick } = require('lodash');
const os = require('os');
...
const clientDependencies = ({ scope, client }) => {
  switch (client) {
    ...
    case 'mysql':
      return {
        'strapi-hook-bookshelf': scope.strapiPackageJSON.version,
        'strapi-hook-knex': scope.strapiPackageJSON.version,
        knex: 'latest',
        [sqlClientModule[client]]: 'latest',
      };
    case 'mongo':
      return {
        'strapi-hook-mongoose': scope.strapiPackageJSON.version,
      };
    default:
...
const OPTIONS_FIELDS = ['ssl', 'authenticationDatabase'];
...
async function askDbInfosAndTest(scope) {
  ...
  async function loop() {
    ...
    const infos = await askDatabaseInfos(scope);
    const { client } = infos;

    const configuration = {
      client,
      connection: merge(defaultConfigs[client] || {}, {
        settings: pick(infos, SETTINGS_FIELDS),
        options: pick(infos, OPTIONS_FIELDS),
      }),
      dependencies: clientDependencies({ scope, client: client }),
    };

    await testDatabaseConnection({ configuration, tmpPath }).catch(err => {
      ...
      throw new Error('Could not connect to your database after 5 tries');
    });
...
async function testDatabaseConnection({ scope, configuration, tmpPath }) {
  ...
  // install dependencies somewhere
  ...
  try {
    ...
    await fse.writeJSON(
      ...
      packageJSON({
        ...
        strapiVersion: scope.strapiPackageJSON.version,
...
async function installDatabaseTestingDep({ scope, configuration, tmpPath }) {
  let packageCmd = scope.hasYarn
    ? `yarnpkg --cwd ${tmpPath} add`
    : `npm install --prefix ${tmpPath}`;
  ...
  await execa.shell(`${packageCmd} ${depArgs.join(' ')}`);
}
